 ...
 */
@Component
public class ArticlePageViewUpdateStrategy extends AbstractArticleStatisticStrategy {

    @Override
    public String getStatisticIndexName() {
        return ArticleEventEnum.PAGE_VIEWS.name().toLowerCase();
    }


    @Override
    public UpdateArticleRequest convertStatisticUpdateRequest(ArticleStatisticsRequest request) {
        return UpdateArticleRequest.builder().pageViews(request.getIncrement()).build();
    }
